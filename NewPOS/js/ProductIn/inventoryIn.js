...
function submit() {
    ...
    if ($('#datepicker').val() == "") {
        ...
        swal('Message', "Empty Supplier", "info").then(() => {
            $('#btn-ser1').html('Submit');
            $("#btn-ser1").attr("disabled", false)
            return;
        })
    } else if ($('#inputGroupSelect01').val() == "0") {
        swal('Message', "Empty Reference No", "info").then(() => {
            $('#btn-ser1').html('Submit');
            $("#btn-ser1").attr("disabled", false)
            return;
        })
        ...
        $(this).children("td").each(function () {
            switch (i) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    obj["quantity"] = parseFloat($(this).find("input").val());
                    break;
                case 3:
                    obj["um"] = $(this).html();
                    break;
    ...
    var eee = customers.find(x => x.sup_Name == array.id);
    ...
    if (verified) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.response === '1') {
                    swal("","Successfully Inserted","success").then(() => {
                        clearData();
                    });
